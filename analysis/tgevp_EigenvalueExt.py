...
def main(cfg: Config):
    ...
    #=============================================================#
    # prepare the data
    # cov = summation of all covariance matrix for each bootstrap
    
    conf = len(exx_list)
    ranksep = rankmax - rankmin + 1
    xcov, ycov = DataExtrapolation.CovarianceMatrix(exx_list, exy_list, conf)
    ...
    for i in range(conf):
        ...
        #=============#
        result = DataExtrapolation.Extrapolation(parameter, exx_list[i], exy_list[i], i, xcov, ycov, ranksep)
